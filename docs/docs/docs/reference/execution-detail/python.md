...
The default list of pip packages and their versions in the sandbox can be found in [this file](https://github.com/bytedance/SandboxFusion/blob/main/runtime/python/requirements.txt). The Python runtime environment is located in a uv-managed virtual environment at `runtime/python/.venv` within the sandbox.
