...
The default list of pip packages and their versions in the sandbox can be found in [this file](https://github.com/bytedance/SandboxFusion/blob/main/runtime/python/requirements.txt). The Python runtime environment is located in a conda environment called `sandbox-runtime` (packages installed via uv). By default, torch is CPU-only; use `install-gpu-runtime.sh` for CUDA support.
